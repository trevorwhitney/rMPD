...
load_tracks = (album)->
  ...
  $.ajax
    ...
    dataType: 'json'
    success: (data)->
      for track, index in tracks.numberless
      ...
      for track, index in tracks.tracks
        template_data =
...
update_current_song = (data)->
  ...
  if current_song == null or data.album != current_song.album
...
timer.onmessage = (message)->
  ...
  if typeof message.time isnt "undefined"
    print_time(message.time.elapsed, message.time.total)
  if typeof message.state isnt "undefined"
